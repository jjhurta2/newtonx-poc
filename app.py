    ...
    for i, lead in enumerate(st.session_state['leads']):
        ...
        if generate_email:
            with st.spinner("Synthesizing relationship context into personalized outreach..."):
                ...
                # Dynamic Instruction Injection Based on Connection Type
                if "Affinity" in lead['connection_type']:
                    ...
                    TONE: Use highly direct, casual language suitable for internal colleagues. Strictly avoid overly formal, stiff, or passive-aggressive phrasing. Get straight to the point as a familiar industry partner.
                    """
                elif "Alumni" in lead['connection_type']:
                    strategy_instruction = """
                    ...
                    TONE: Use highly direct, casual language suitable for internal colleagues. Strictly avoid overly formal, stiff, or passive-aggressive phrasing. Get straight to the point.
                    """
                else:
                    strategy_instruction = """
                    TASK: Write a cold email DIRECTLY to the Target Prospect. (Do NOT address or mention the Bridge Contact).
                ...
                Bridge Contact (Your existing relationship): {lead['bridge_name']} ({lead['bridge_role']})
                Relationship Details: {lead['rationale']}
                
                {strategy_instruction}
                
                CRITICAL RULES:
                - Output exactly ONE email draft.
                - Write ONLY the email body. Do not include subject lines or conversational filler before/after the draft.
